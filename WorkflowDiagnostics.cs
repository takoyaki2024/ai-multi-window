        ...
        try
        {
            ...
            if (engine is not null)
            {
                text.AppendLine($"STATE: {engine.State}")
                    ...
                    .AppendLine($"PREVIOUS_PATCH_SEARCH_HASH: {Clean(engine.PreviousPatchSearchHash)}")
                    .AppendLine($"RETRY_STRATEGY: {Clean(engine.RetryStrategy)}")
                    .AppendLine($"CODER_REPAIR_ATTEMPT: {engine.CoderRepairAttempt}/{engine.MaxFixAttempts}")
                    .AppendLine($"SAFE_PATCH_HINT_AVAILABLE: {!string.IsNullOrWhiteSpace(engine.SafePatchHint)}")
                    .AppendLine($"SAFE_PATCH_HINT_FILE: {Clean(engine.SafePatchHintFile)}")
                    .AppendLine($"SAFE_PATCH_HINT_HASH: {Clean(engine.SafePatchHintHash)}")
                    .AppendLine($"CODER_OUTPUT_FORMAT_REPAIR: {engine.CoderOutputFormatRepair}")
